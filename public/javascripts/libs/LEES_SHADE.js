...
function valid(obj){
	   ...
	   if(arguments[1]){
		   ...
		   for(var va in o){
		       if(obj[va]){
			       if(o[va].pattern){obj[va].pattern = o[va].pattern;}
			       else if(obj[va].pattern){}
				   else if(obj[va].type){}
				   else{alert(va+" has no pattern or type!");}
				   obj[va].va =va;
				   if(o[va].type == "equal"){
				       if(o[va].tagelename){
					       $(obj[va]).blur(function(){
						       if($(obj[o[this.va].tagelename]).val() == $(this).val()){
							       _valid(this);
							   }
							   else {
							       _invalid(this);
						  ...
						  $(obj[va]).focus(function(){
				          	 _focus(this);
							 $(this).val("");
				   			}); //focus function end
					   }//end if(o.value)
					   else {alert("no target element error")}
				   }//end if
				   else {
					
					if(obj[va].length){var len = obj[va].length;}
					else {var len = 1;}
					 ...
					 if(len==1){_o = $(obj[va]);}
						 ...
						 _op = obj[va];
						 _op[len-1].pattern = _op[0].pattern;
						 _o = $(_op[len-1]);
					 }
				   //onchange event
			       _o.blur(function(){
	   ...
	   else{alert("error,arguments are not enough!")}
	          function _valid(obj){ 
	                       if($("#msg_"+obj.va).hasClass("invalid"))$("#msg_"+obj.va).removeClass("invalid");
					       $("#msg_"+obj.va).addClass("valid");
						   if(o[obj.va].validtext){$("#msg_"+obj.va).html(o[obj.va].validtext);}
						   else if(o["default"].validtext){$("#msg_"+obj.va).html(o["default"].validtext);}
						   else {$("#msg_"+obj.va).html("");}
	          }//end _valid
			  function _invalid(obj){
			  			   if($("#msg_"+obj.va).hasClass("valid"))$("#msg_"+obj.va).removeClass("valid");
					       $("#msg_"+obj.va).addClass("invalid");
						   if(o[obj.va].invalidtext){$("#msg_"+obj.va).html(o[obj.va].invalidtext);}
						   else if(o["default"].invalidtext){$("#msg_"+obj.va).html(o["default"].invalidtext);}
						   else {$("#msg_"+obj.va).html("");}
			  }//end _invalid
			  function _focus(obj){
			               if($("#msg_"+obj.va).hasClass("invalid"))$("#msg_"+obj.va).removeClass("invalid");
						   if($("#msg_"+obj.va).hasClass("valid"))$("#msg_"+obj.va).removeClass("valid");
						   if(o[obj.va].focustext){$("#msg_"+obj.va).html(o[obj.va].focustext);}
						   else if(o["default"].focustext){$("#msg_"+obj.va).html(o["default"].focustext);}
						   else {$("#msg_"+obj.va).html("");}
			  }//end _focus
